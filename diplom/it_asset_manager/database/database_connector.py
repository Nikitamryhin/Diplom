...
class DatabaseConnector:
    def __init__(self, db_path):
        ...
        self.connection = None  # Изменил conn на connection
        self.cursor = None
        self.connect()
        self.create_tables()
    ...
    def connect(self):
        try:
            self.connection = sqlite3.connect(self.db_path)  # Используем self.connection
    ...
    def create_tables(self):
        try:
            ...
            """)
            self.connection.commit()  # Используем self.connection
    ...
    def insert_device(self, name, category, serial_number, status, department_id, employee_id):
        ...
        try:
            self.cursor.execute("""
                INSERT INTO devices (name, category, serial_number, status, department_id, employee_id)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (name, category, serial_number, status, department_id, employee_id))
            self.connection.commit()  # Используем self.connection
