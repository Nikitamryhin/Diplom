...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        # ---  Главное меню ---
        self.main_menu = tk.Menu(self)  # Сначала создаем self.main_menu
        self.config(menu=self.main_menu)  # Затем используем его
        ...
        # ---  Меню Отчеты ---
        reportmenu = tk.Menu(filemenu, tearoff=0)
        reportmenu.add_command(label="Сформировать отчет по отделу", command=self.generate_department_report)
        filemenu.add_cascade(label="Отчеты", menu=reportmenu)
    ...
    def generate_department_report(self):
            if self.selected_department_id is None:
                messagebox.showinfo("Внимание", "Выберите отдел для создания отчета.")
                return

            self.generate_report(self.selected_department_id)

    # --- Отчетность ---
    def generate_report(self, department_id=None):
        """Генерирует отчет в окне Tkinter."""

        # 1. Получаем данные из базы данных
        if department_id:
            departments = [database.get_department(department_id)]
        else:
        ...
        report_text = "Отчет о состоянии компьютерной техники:\n\n"
