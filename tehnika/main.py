...
class MainWindow(tk.Tk):
    ...
    def create_ui(self):
        ...
        #  --- Список установленного ПО (справа снизу) ---
        self.software_label = ttk.Label(self.info_frame, text="Установленное ПО:")
        self.software_label.pack(pady=5)

        self.software_tree = ttk.Treeview(self.info_frame, columns=("name", "version"), show="headings")
        self.software_tree.heading("name", text="Название")
        self.software_tree.heading("version", text="Версия")
        self.software_tree.pack(fill="both", expand=True, padx=5, pady=5)

        # Фрейм для кнопок "Добавить ПО" и "Удалить ПО"
        self.software_buttons_frame = ttk.Frame(self.info_frame)
        self.software_buttons_frame.pack(pady=5, anchor=tk.CENTER)

        # Кнопка "Добавить ПО"
        self.add_software_button = ttk.Button(self.software_buttons_frame, text="Добавить ПО", command=self.add_software)
        self.add_software_button.pack(side=tk.LEFT, padx=2)

        # Кнопка "Удалить ПО"
        self.delete_software_button = ttk.Button(self.software_buttons_frame, text="Удалить ПО", command=self.delete_software)
        self.delete_software_button.pack(side=tk.LEFT, padx=2)
    ...
    def delete_software(self):
        """Удаляет выбранное ПО для выбранного устройства."""
        try:
            selected_item = self.software_tree.selection()[0]
            software_name = self.software_tree.item(selected_item)['values'][0] # Получаем имя ПО
            software_version = self.software_tree.item(selected_item)['values'][1] # Получаем версию ПО

            if self.selected_device_id:
                if messagebox.askyesno("Удаление ПО", f"Вы уверены, что хотите удалить {software_name}?", parent=self):
                    database.delete_software_from_device(self.selected_device_id, software_name, software_version)
                    self.load_device_software() # Обновляем список ПО
            else:
                messagebox.showinfo("Внимание", "Выберите устройство для удаления ПО.", parent=self)
        except IndexError:
            messagebox.showinfo("Внимание", "Выберите ПО для удаления.", parent=self)
    ...
    def load_devices(self):
        ...
        if self.selected_department_id:
            ...
            for device in devices:
                ...
                device[2],  # Тип
                device[3],  # Модель
                device[4],  # Серийный номер
                device[5],  # Инвентарный номер
                device[6],  # CPU
                device[7],  # Memory
                device[8],  # HDD
                device[9],  # GPU
    ...
    def load_device_info(self):
        """Загружает информацию об устройстве в правую часть окна"""
        if self.selected_device_id:
            ...
            if device:
                ...
                self.load_device_software()  # Загружаем список ПО для устройства
            else:
    ...
    def clear_device_info(self):
        ...
        self.status_entry.delete(0, tk.END)
        self.save_button["state"] = tk.DISABLED
        self.clear_device_software()

    def clear_device_software(self):
        """Очищает Treeview со списком ПО"""
        for item in self.software_tree.get_children():
            self.software_tree.delete(item)

    def load_device_software(self):
        """Загружает список установленного ПО для выбранного устройства"""
        # Очищаем Treeview
        for item in self.software_tree.get_children():
            self.software_tree.delete(item)

        # Получаем список ПО из базы данных
        if self.selected_device_id:
            software_list = database.get_software_for_device(self.selected_device_id)
            # Заполняем Treeview
            for software in software_list:
                self.software_tree.insert("", tk.END, values=(software[0], software[1]))

    def add_software(self):
        """Открывает диалоговое окно для добавления ПО"""
        if self.selected_device_id:
            AddSoftwareDialog(self, self.selected_device_id)
        else:
            messagebox.showinfo("Внимание", "Выберите устройство для добавления ПО.", parent=self)
    ...
    def save_device(self):
        ...
        if self.selected_device_id:
            ...
            #  Обновляем ТОЛЬКО выбранную строку в Treeview
            selected_item = self.computer_tree.selection()[0] #  Получаем выбранную строку

            self.computer_tree.item(selected_item, values=(
                self.selected_device_id,  # ID остается прежним
                device_type,
                model,
                serial_number,
                inventory_number,
            ...
            self.load_device_info() #Обновление информации
    ...
    def on_device_select(self, event):
        ...
        try:
            ...
            self.load_device_software() #  Загружаем список ПО вместо периферии !!!
        except IndexError:
            self.clear_device_info()
            self.clear_device_software()
            self.selected_device_id = None
...
class AddSoftwareDialog(simpledialog.Dialog):
    def __init__(self, parent, device_id):
        self.parent = parent
        self.device_id = device_id
        super().__init__(parent, title="Добавить ПО")
    ...
    def body(self, master):
        ...
        ttk.Label(master, text="Версия:").grid(row=1, sticky=tk.W)
        self.version_entry = ttk.Entry(master)
        self.version_entry.grid(row=1, column=1, sticky=tk.E)
    ...
    def apply(self):
        ...
        database.add_software_to_device(self.device_id, name, version)
        self.parent.load_device_software()  # Обновляем список ПО
            ...
            for department in departments: #  Перебираем все отделы
                ...
                if devices:
                    ...
                    for device in devices:
                        ...
                        report_text += f"  Тип: {device[1]}\n"
                        report_text += f"  Модель: {device[2]}\n"
                        report_text += f"  Серийный номер: {device[3]}\n"
                        report_text += f"  Инвентарный номер: {device[4]}\n"
                        report_text += f"  CPU: {device[5]}\n"
                        report_text += f"  Memory: {device[6]}\n"
                        report_text += f"  HDD: {device[7]}\n"
                        report_text += f"  GPU: {device[8]}\n"
                        report_text += f"  Статус: {device[9]}\n\n"
                else:
