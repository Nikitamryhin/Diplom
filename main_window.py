...
class MainWindow:
    ...
    def load_department_data(self, department_id):
        """Загружает устройства и сотрудников для выбранного отдела."""
    ...
    def load_employees(self, department_id=None):
        ...
        try:
            ...
            if department_id is None:
                employees = self.db_connector.fetch_all_employees()  # Изменено
            else:
    ...
    def open_add_device_window(self):
        add_window = AddDeviceWindow(self.root, self.db_connector, self.load_devices)
